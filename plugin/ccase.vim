...
" Created:              17-Feb-2000
" Last Modified:        22-Sep-2005 09:36
"
" GetLatestVimScripts: 15 1 ccase.vim
"
" $Id: ccase.vim,v 1.40 2005/11/30 19:36:28 dp Exp $ }}}
"
" Modifications: {{{
" $Revision: 1.40 $
" $Log: ccase.vim,v $
" Revision 1.40  2005/11/30 19:36:28  dp
" Changes from arch/CVS synchronization
"
" Revision 1.39  2005/08/10 17:25:18  dp
" Added ability to show file's version in the statusline
"
...
augroup ccase
  ...
  au BufNewFile,BufRead * silent exe s:CtGetVersion(expand("%"))

         "if exists('g:ccaseDispVerInStatus') == 1 && g:ccaseDispVerInStatus == 1 |
        "\ let b:ccaseVersion = system("cleartool desc -fmt %Sn " . expand("%"))
        "\ endif"
...
" Allow user to enter comments into a separate window and use the
" '-cfile <comment_file>' option.
if !exists("g:ccaseUseFileComment")
  let g:ccaseUseFileComment = 0 " Default is to not use a file for comments
endif
...
" Allow customization of how the statusline is displayed other than the
" ClearCase aspects.
if !exists("g:ccaseDefBegStatusLine")
  let g:ccaseDefBegStatusLine = "%<%f%h%m%r%="
endif
if !exists("g:ccaseDefEndStatusLine")
  let g:ccaseDefEndStatusLine = "%l,%c%V\ %P"
endif

"let statusline="%<%f%h%m%r%=%{$view}\ %{&ff}\ %l,%c%V\ %P"
let s:statusline=g:ccaseDefBegStatusLine

" Setup statusline to show current view, if your environment sets the
" $view variable when inside a view.
if exists("$view")
  let s:statusline = s:statusline . "%{$view}\ "
endif

" If enabled, display the file's ClearCase version in the statusline
if !exists("g:ccaseDispVerInStatus")
  let g:ccaseDispVerInStatus = 0
endif

if g:ccaseDispVerInStatus == 1
  let s:statusline = s:statusline . "%{CtVersionExist('b:ccaseVersion')} "
endif
...
" ===========================================================================
function! s:CtGetVersion(fname)
" Shell out and get the requested file's version from ClearCase
" ---------------------------------------------------------------------------
  if exists('g:ccaseDispVerInStatus') == 1 && g:ccaseDispVerInStatus == 1
    let b:ccaseVersion = system("cleartool desc -fmt %Sn " . expand("%"))
  endif
...
" ===========================================================================
function! CtVersionExist(var)
" Return the version string for the current buffer if it exists.
" ---------------------------------------------------------------------------
  if exists(a:var)
    return b:ccaseVersion
  else 
    return '' 
  endif
endfunction

let &statusline=s:statusline . g:ccaseDefEndStatusLine
...
" Current revision:
let s:revision =
  \ substitute("$Revision: 1.40 $",'\$\S*: \([.0-9]\+\) \$','\1','')
...
*ccase.txt*	For Vim version 6.0 and up                           #version#
             LAST MODIFICATION: "Thu, 22 Sep 2005 09:36:42 (dp)"
